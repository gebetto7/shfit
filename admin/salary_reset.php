<?php
    ...
    for ($count = 0; $count <= sizeof($staff_array["staff"]); $count++){
        $url = "../data/salary/salary" . $count . ".json";
        $json = file_get_contents($url);
        $sum = json_decode($json, true);

        $sum["salary"][0]["weekly_hours"] = 0;
        $sum["salary"][0]["weekly_days"] = 0;

        $sum["salary"][1]["monthly_hours"] = 0;
        $sum["salary"][1]["monthly_days"] = 0;

        $sum["salary"][2]["yearly_hours"] = 0;
        $sum["salary"][2]["yearly_days"] = 0;
